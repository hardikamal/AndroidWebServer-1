...
public class ServerService extends Service {
    ...
    public void startServer(Handler handler) {
        ...
        try {
            ...
            isLoopback = sharedPreferences.getBoolean(getString(R.string.pk_config_loopback_ip), true);
            // Start server
            isRunning = true;
            if (isWifiAPEnabled && !isLoopback) {
                ipAddress = getAPIpAddress();         
            } else {
                ipAddress = intToIp(wifiInfo.getIpAddress());
            }
            int port = Integer.valueOf(sharedPreferences.getString(getString(R.string.pk_port), "8080"));


            if (((!wifiManager.isWifiEnabled()) || (wifiInfo.getSupplicantState() != SupplicantState.COMPLETED) || (wifiInfo.getIpAddress() == 0)) && !isWifiAPEnabled || isLoopback) {
                ipAddress="127.0.0.1";
            ...
            MainActivity.putToLogScreen("E: " + e.getMessage(), gHandler);
        }
    }
